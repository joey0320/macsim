...
Please take a look at the readme in macsim_src_dir/tools/gpu_tracegen
...
1. set up ```trace_file_list``` to the ```kernel_config.txt``` path
2. set up ```params.in```
3. use slurm to run the process
  - ```sbatch run.sh```
4. parse the data by using ```get_stats.py```
  - ```python3 get_stats.py --result=./results --outfile=./out-dir/out.csv```
